...
std::vector<std::vector<int>> tag_pixels(int width, int height, std::vector<std::vector<int>> kmeans, std::vector<int> image, std::vector<int> tags) {
    ...
    for (int i = 0; i < NUM_KMEANS; i++) {
            ...
            double blue = 0;
            int x,y;
            //#pragma omp parallel for reduction(+:red,green,blue) private(x) num_threads(4)
            for (y = 0; y < height; y++) {
                for (x = 0; x < width; x++) {
            ...
            modifiedKmeans[i] = std::vector<int>{static_cast<int>(red), static_cast<int>(green), static_cast<int>(blue), i};
    }
...
int findClosestCluster(const Vector3D& vector, const std::vector<std::vector<int>>& kmeans) {
    int tag = -1;
